...
uint8_t adjust_flag = 0;		//flag to adjust time
uint8_t input_flag = 0;
uint8_t pm_flag = 0;			//flag for am or pm
uint8_t hour24_flag = 0;		//flag to do 24 hour mode
uint8_t lcd_flag = 0;			//flag to update lcd
uint8_t manual_brightness = 0;		//flag to adjust brightness
uint8_t trigger_alarm = 0;		//flag to sound alarm
uint8_t adjust_alarm = 0;		//flag to adjust alarm time
uint8_t alarm_match_count = 0;		//flag to see if alarm should trigger
uint8_t alarm_is_set = 0;		//flag for when it is buzzing
uint8_t alarm_set_once = 0;		//flag to only do alarm once
volatile uint8_t isr_count = 0;		//variable to count isr times
volatile uint8_t sec_count = 0;		//variable to count seconds
volatile uint8_t min_count = 0;		//variable to count minutes
volatile uint8_t hour_count = 0;	//variable to count hours

uint8_t temp_min = 0;			//variable to hold temp minutes
uint8_t temp_hour = 0;			//variable to hold temp hours
uint8_t temp_pm_flag = 0;		//variable to hold temp am or pm
uint8_t alarm_time_sec = 0;		//alarm count in seconds
uint8_t alarm_time_min = 0;		//alarm count in minutes
uint8_t alarm_time_hour = 0;		//alarm count in hours
uint8_t ten_sec_start = 0;		//count the 10 seconds
uint8_t ten_sec_count = 0;		//initialize the 10 sec snooze

uint8_t temp_read_flag = 0x01;		//flag to read local temp
